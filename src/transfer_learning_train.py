...
RUN_WITH_NNI = False # For this script I do not see a usecase for this flag to be True
device = 'cuda'
NUM_DEVICES = 1
NUM_NODES = 1
LEARNING_RATE = 1e-4
...
IMAGE_HEIGHT = 256
IMAGE_WIDTH = 256
TRAIN_IMG_DIR = "../cloudcontainer/satellite/full_dataset/train/images/"
TRAIN_MASK_DIR = "../cloudcontainer/satellite/full_dataset/train/masks/"
VAL_IMG_DIR = "../cloudcontainer/satellite/full_dataset/val/images/"
VAL_MASK_DIR = "../cloudcontainer/satellite/full_dataset/val/masks/"
TEST_IMG_DIR = "../cloudcontainer/satellite/full_dataset/test/images"
TEST_MASK_DIR = "../cloudcontainer/satellite/full_dataset/test/masks"
TEST_DEMO_DIR = "../cloudcontainer/satellite/saved_transfer_learning_test_demos/"
VAL_DEMO_DIR = "../cloudcontainer/satellite/saved_transfer_learning_val_demos/"
...
IMAGE_HEIGHT = 256
IMAGE_WIDTH = 256
TRAIN_IMG_DIR = "../cloudcontainer/satellite/local_dataset_subset/train/images/"
TRAIN_MASK_DIR = "../cloudcontainer/satellite/local_dataset_subset/train/masks/"
VAL_IMG_DIR = "../cloudcontainer/satellite/local_dataset_subset/val/images/"
VAL_MASK_DIR = "../cloudcontainer/satellite/local_dataset_subset/val/masks/"
TEST_IMG_DIR = "../cloudcontainer/satellite/local_dataset_subset/test/images"
TEST_MASK_DIR = "../cloudcontainer/satellite/local_dataset_subset/test/masks"
TEST_DEMO_DIR = "../cloudcontainer/satellite/saved_transfer_learning_test_demos/"
VAL_DEMO_DIR = "../cloudcontainer/satellite/saved_transfer_learning_val_demos/"
"""
...
class DeadSeaUNet(pl.LightningModule):
    ...
    def setup(self, stage):
        ...
        self.best_val_loss = torch.tensor([1000000000])
    ...
    def on_validation_epoch_end(self):
        ...
        if avg_val_loss.item() <= self.best_val_loss.item():
            self.best_val_loss = avg_val_loss
            print("Found new best validation loss equal to ", self.best_val_loss.item())
            self.trainer.save_checkpoint(f"../cloudcontainer/satellite/transfer_learning_models/tr_best_fine_tuned_model.ckpt")

            if self.current_epoch % 5 == 0:
                print("Saving new best checkpoint")
                self.trainer.save_checkpoint(f"../cloudcontainer/satellite/transfer_learning_models/checkpoints/tr_checkpoint_model_bs{self.batch_size}_ep{self.current_epoch}_valloss{avg_val_loss:.2f}_valdice{avg_val_dice_score:.2f}.ckpt")
                # tr_ stands for transfer learning

        return {'val_loss': avg_val_loss, 'avg_val_dice_score': avg_val_dice_score, 'avg_val_iou': avg_val_iou}

    def on_train_end(self):
        print("Training is finished, the best validation loss was: ", self.best_val_loss.item())
        ...
        best_val_model = DeadSeaUNet.load_from_checkpoint("../cloudcontainer/satellite/transfer_learning_models/tr_best_fine_tuned_model.ckpt", n_channels=args['in_channels'], n_classes=args['out_classes'], batch_size=args['batch_size'])
...
def main(args):
    ...
    transfer_learning_init_checkpoint_path = "../cloudcontainer/satellite/transfer_learning_models/best_nonweightedCE_initialization_model.ckpt"
    transfer_learning_model = DeadSeaUNet.load_from_checkpoint(transfer_learning_init_checkpoint_path, n_channels=args['in_channels'], n_classes=args['out_classes'], batch_size=args['batch_size'])
    
    # Freeze the initial feature extraction blocks, but leave the more complex feature blocks unfrozen to be fine-tuned
    for nr_block, param in enumerate(transfer_learning_model.parameters()):
        print(f"Block {nr_block} has {param.shape} params")
        if nr_block <= 5:
            param.requires_grad = False

    # Check visually what blocks of parameters were frozen:
    print("\nFrozen vs unfrozen status of parameter blocks:\n")
    ...
    trainer.fit(transfer_learning_model)

    # Qualitative evaluation of the best model:
    test_tr_model = DeadSeaUNet.load_from_checkpoint("../cloudcontainer/satellite/transfer_learning_models/tr_best_fine_tuned_model.ckpt", n_channels=args['in_channels'], n_classes=args['out_classes'], batch_size=args['batch_size'])
    trainer.test(test_tr_model)


if __name__ == '__main__':
    os.makedirs("../cloudcontainer/satellite/transfer_learning_models", exist_ok=True)
    ...
    if RUN_WITH_NNI:
        ...
        parser.add_argument('--logs_dir', type=str, default='../cloudcontainer/satellite')
